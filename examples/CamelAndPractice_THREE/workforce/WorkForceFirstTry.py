...
from camel.toolkits import SearchToolkit, search_toolkit
...
#1.创建一个WorkForce实例
workforce = Workforce(description="旅游攻略制作与评估工作组",
...
#新增加的一个Agent,用于节省旅游中的预算
budget_agent = ChatAgent(
    system_message="""你是一个专业的旅游预算助手。你的职责是:
        1. 根据行程计划和景点信息估算成本
        2. 考虑因素如交通、住宿、美食等
        3. 提供合理的成本估算
        请根据行程计划和景点信息进行估算。""",
...
#3.添加工作节点
#添加一个执行网页搜索的Agent(这里是连续添加多个，使用链式添加规则)
